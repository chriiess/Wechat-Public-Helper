...
function GetMsg($msg) {
    ...
    // $content = Curl_get("http://api.douqq.com/?key=UjZDaHVjPVZQMDNwTmt4PUFuOUlMc3lzQlFvQUFBPT0&msg=" . $msg);
    // $content = ClearHtml($content);
    // $content = strCut($content, 1600, '...');
    // return $content;

    // $content = Curl_get("http://api.wbscms.com/api.php?msg=" . $msg);
    // $content = json_decode($content);
    // return $content->wbscmsapi;

    $content = Curl_get("http://sandbox.api.simsimi.com/request.p?key=5b11af18-fba7-45e9-9e4d-d78d7d7a3c74&lc=ch&ft=1.0&text=" . urlencode($msg));
    $content = json_decode($content);
...
function GirlPic() {
    ...
    $id = rand(0, 940);
    $url = "http://www.tngou.net/tnfs/api/news?id=$id&rows=6";
    $info = Curl_get($url);
    $jsonData = json_decode($info);
    $tngou = $jsonData->tngou;

    $data = array();

    foreach ($tngou as $key => $value) {

        $data['items'][$key]['title'] = $tngou[$key]->title;
        $data['items'][$key]['description'] = $tngou[$key]->title;
        $data['items'][$key]['picurl'] = "http://tnfs.tngou.net/image" . $tngou[$key]->img;
        $data['items'][$key]['url'] = "http://tnfs.tngou.net/image" . $tngou[$key]->img;
    }
